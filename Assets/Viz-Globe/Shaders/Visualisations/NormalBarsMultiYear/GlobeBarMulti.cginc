...
void geom(point v2g input[1], inout TriangleStream<g2f> triStream)
{
	...
	float ActiveYear = UNITY_ACCESS_INSTANCED_PROP(Props, _ActiveYear);
	...
	float val1 = input[0].uv.x; //2000
	float val2 = input[0].uv.y; //2005
	float val3 = input[0].uv2.x; //2010
	float val4 = input[0].uv2.y; //2015
	float val5 = input[0].uv3.x; //2020

	float3 vertex = input[0].vertex.xyz;
	float range, val = 0;

	float decimal = frac(ActiveYear);
	int active = trunc(ActiveYear);

	switch (active) {
	case 0:
		val = lerp(0, val1, decimal);
		break;
	case 1:
		val = lerp(val1, val2, decimal);
		break;
	case 2:
		val = lerp(val2, val3, decimal);
		break;
	case 3:
		val = lerp(val3, val4, decimal);
		break;
	case 4:
		val = lerp(val4, val5, decimal);
		break;
	case 5:
		val = val5;
		break;
