...
public static class ColorBru
{
    ...
    public static Color[] GetColors(Code code, byte numberOfColors, bool reversed)
    {
        List<Color> availabeColors = new List<Color>();
        int nCol = 0;
        foreach(Palette pal in Palettes)
        {
            if(pal.Code == code)
            {
                string[] htmls = pal.HtmlCodes[pal.HtmlCodes.Count - 1];
                nCol = htmls.Length;
                for (int i = 0; i < htmls.Length; i++)
                {
                    string html = htmls[i];
                    availabeColors.Add(ColorTranslator.FromHtml(html));
                }
                break;
            }
        }

        int nmax = (int)Mathf.Min(numberOfColors, nCol);
        if (reversed)
        {
            return availabeColors.GetRange(0, nmax).Reverse<Color>().ToArray();
        }
        else
        {
            return availabeColors.GetRange(0, nmax).ToArray();

        }
    }

    public static Vector4[] ColorsToVector4s(Color[] colors)
    {
        Vector4[] vecs = new Vector4[colors.Length];
        for(int i = 0; i < colors.Length; i++)
        {
            Color c = colors[i];
            vecs[i] = new Vector4(c.r, c.g, c.b, c.a);
        }
...
public static class ColorTranslator
{
    ...
    {
        Color c = new Color();
        ColorUtility.TryParseHtmlString(html, out c);
